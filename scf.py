...
#mol = Molecule(filename=geometry,basis='sto-3g',load=load,gauge=[7423.0,2380.0,3450.0])
#mol = Molecule(filename=geometry,basis='sto-3g',load=load,gauge=[0.0,0.0,0.0])
#mol = Molecule(filename=geometry,basis='sto-3g',load=load,gauge=[50.0,0.0,50.0])
...
#print np.dot(mol.adj(mol.C),np.dot(mol.S,mol.C))
#RT = False 
RT = True 
#mol.buildFock()
#mol.computeEnergy()
...
if RT:
    ...
    #plt.plot(np.arange(len(mol.Hbmom)),2*np.asarray(mol.Hbmom)+np.asarray(mol.Gbmom)-2*np.asarray(mol.Sbmom),label='2H + G - S')
