import numpy as np
from mmd.grid.lebedev import lebedev

class atomic_grid(object):
    def __init__(self,atom,**kwargs):
        atno,x,y,z = atom.charge,atom.origin[0],atom.origin[1],atom.origin[2]
...
def LegendreGrid(Z,**kwargs):
    from mmd.grid.data import Bragg
    from mmd.grid.legendre import legendre
    Rmax = 0.5*Bragg[Z]*1.8897259886 # Bragg is in angstrom, we want bohr
...
def EulerMaclaurinRadialGrid(nrad,Z):
    from mmd.grid.data import PopleRadii
