...
def becke_atomic_grid_p(ati,xyzp,atoms,**kwargs):
    ...
    sprod = 1
    rip = np.linalg.norm(ati.origin-xyzp)
    for atj in atoms:
        if ati == atj: continue
        rij = np.linalg.norm(ati.origin-atj.origin)
        rjp = np.linalg.norm(atj.origin-xyzp)
        mu = (rip-rjp)/rij
        # Modify mu based on Becke hetero formulas (App A)
        if do_becke_hetero and ati.charge != atj.charge:
            chi = Bragg[ati.charge]/Bragg[atj.charge]
            u = (chi-1.)/(chi+1.)
            a = u/(u*u-1)
            a = min(a,0.5)
            a = max(a,-0.5)
